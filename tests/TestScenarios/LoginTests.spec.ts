...
test.describe("LoginPage test scenarios",()=> {
    ...
    test.only('Other Roles User Login Tests', async ({ page }) => {
        await page.goto(`${loginPage.pageUrl}`);
        const users = [
            {email: process.env['Supplier1YoneticisiEmail'] ?? '', password: process.env['Supplier1YoneticisiPassword'] ?? ''},
            {email: process.env['Supplier1CalisaniEmail'] ?? '', password: process.env['Supplier1CalisaniPassword'] ?? ''}/*,
